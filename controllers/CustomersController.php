...
class CustomersController extends Controller
{
    public function actionIndex()
    {
        ...
        #$records = $this->findAllRecordsByQuery();
    ...
    private function getAllRecordsByPhoneNumber($number)
    {
        $phone_record = PhoneRecord::findAll(['number' => $number]);
        if (!$phone_record)
            return [];

        $customer_record = CustomerRecord::findAll($phone_record->customer_id);
        if (!$customer_record)
            return [];
    ...
    private function findAllRecordsByQuery()
    {
        $number = Yii::$app->request->get('phone_number');
        $records = $this->getAllRecordsByPhoneNumber($number);
        $dataProvider = $this->wrapIntoDataProvider($records);
        return $dataProvider;
    }
}